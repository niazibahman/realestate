    ...
    return(<div className="shadow-radika w-full rounded-lg grid grid-cols-1 lg:grid-cols-5 gap-0 my-10">
        <div className="col-span-1 lg:col-span-2 lg:col-start-4 rounded-r-lg">
            <div className="relative w-full h-64 rounded-r-lg">
                <Link href={property.issell === true ? ("/detail/sel-" + property.sellId) : ("/detail/let-" + property.letId)} className="w-full"><Image src={property.photos[0]} alt={property.slug} width={0} height={0} sizes='100%' className="w-full h-full object-cover rounded-t-lg lg:rounded-t-none lg:rounded-tl-lg lg:rounded-bl-lg"/></Link>
                <Link href={property.issell === true ? ("/detail/sel-" + property.sellId) : ("/detail/let-" + property.letId)}><div className="absolute w-full h-2/3 bottom-0 inset-x-0 bg-gradient-to-b from-transparent to-listingCardbg bg-opacity-5 lg:rounded-bl-lg"/></Link>
        ...
        <div className="col-span-1 lg:col-span-3 lg:col-start-1 lg:row-start-1 rounded-b-lg h-auto flex flex-col justify-between">
            <div className="w-full px-4 py-5 flex flex-row items-center justify-between">
                <div className="flex flex-col">
                    <Link href={property.issell === true ? ("/detail/sel-" + property.sellId) : ("/detail/let-" + property.letId)}><h2 className="font-bold text-lg text-textColorH">{property.slug}</h2></Link>
            ...
            <div className="w-full px-4 py-5 flex flex-row items-center justify-between border-t">
                ...
                <Link href={property.issell === true ? ("/detail/sel-" + property.sellId) : ("/detail/let-" + property.letId)} className="bg-greenTheme text-white py-2 px-3 rounded-sm">اطلاعات بیشتر</Link>
            </div>
        </div>
    </div>);
}