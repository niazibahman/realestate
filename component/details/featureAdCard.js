...
export default function FeatureAdCard({property}){
    return(
        <div className="w-full p-4 bg-white rounded border border-detailBorder flex flex-row items-center mb-4">
            <Image src={property.photos[0]} alt={property.slug} width={110} height={120} className="rounded object-cover"/>
            <div className="w-full flex flex-col justify-between mr-3">
                ...
                <div className="w-full flex flex-row items-center justify-between py-2">
                    <span className={`w-16 py-1 text-center text-sm rounded-xl ${property.issell === true ? "text-detailFeatureAdSell bg-detailFeatureAdSell bg-opacity-10":"text-detailFeatureAdRent bg-detailFeatureAdRent bg-opacity-10"}`}>{property.issell === true ?"فروش":"اجاره"}</span>
